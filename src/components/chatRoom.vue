  ...
  <section class="content-wrap">
    ...
    <div class="chat-input-wrap">
      <form method="POST" action="" name="chat-frm" class="chat-frm">
        <span @click="changeType" class="chat-type" :class="chatType">{{
          chatType
        }}</span>
        <textarea
          v-model="msg"
        ...
        <button @click.prevent="submitMsg" class="chat-send">send</button>
      </form>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    chatWrapScroll() {
      if (
        this.$refs.chatWrap.scrollTop <=
        this.$refs.chatWrap.scrollHeight - 630
      ) {
